...
// Per-thread state for concurrent executions of the same benchmark.
        ...
        void Run() {
            ...
            while (benchmarks != nullptr) {
                ...
                if (name == Slice("open")) {
                    ...
                    range_ = FLAGS_num / FLAGS_range_size;
    ...
    private:
        ...
        // Add Range query performance test
        void RangeQuery(ThreadState *thread) {
            ReadOptions options;
            std::string value;
            int64_t bytes = 0;
            for (int i = 0; i < range_; i++) {
                const int begin_pos = abs((int) (thread->rand.Next() % FLAGS_num) - range_size_);
                const int end_pos = begin_pos + range_size_;
                char begin[100];
                int state = 0;
                state = snprintf(begin, sizeof(begin), "%016d", begin_pos);
                if (state == -1) {
